            ...
            <div class="filters-grid">
                <div class="filter-item">
                    ...
                    <select name="status[]" id="status" class="form-control select2-multiple" multiple>
                        <option value="pending" {{ is_array(request('status')) && in_array('pending', request('status')) ? 'selected' : '' }}>Pending</option>
                        <option value="active" {{ is_array(request('status')) && in_array('active', request('status')) ? 'selected' : '' }}>Active</option>
                        <option value="completed" {{ is_array(request('status')) && in_array('completed', request('status')) ? 'selected' : '' }}>Completed</option>
                        <option value="cancelled" {{ is_array(request('status')) && in_array('cancelled', request('status')) ? 'selected' : '' }}>Cancelled</option>
                    </select>
                </div>
                ...
                <div class="filter-item">
                    ...
                    <select name="location[]" id="location" class="form-control select2-multiple" multiple>
                        @foreach($locations as $location)
                            <option value="{{ $location }}" {{ is_array(request('location')) && in_array($location, request('location')) ? 'selected' : '' }}>{{ $location }}</option>
                        @endforeach
                    </select>
                </div>
                
                <div class="filter-item">
                    <label for="client_name">Company/Client</label>
                    <select name="client_name[]" id="client_name" class="form-control select2-multiple" multiple>
                        @foreach($clients as $client)
                            <option value="{{ $client }}" {{ is_array(request('client_name')) && in_array($client, request('client_name')) ? 'selected' : '' }}>{{ $client }}</option>
                        @endforeach
                    </select>
                </div>
                
                <!-- Dynamic Service Type Filter -->
                <div class="filter-item">
                    <label for="service_type_id">Service Type</label>
                    <select name="service_type_id[]" id="service_type_id" class="form-control select2-multiple" multiple>
                        @foreach($serviceTypes as $id => $name)
                            <option value="{{ $id }}" {{ is_array(request('service_type_id')) && in_array($id, request('service_type_id')) ? 'selected' : '' }}>{{ ucfirst($name) }}</option>
                        @endforeach
                    </select>
                </div>
                
                <!-- New Tender ID Filter -->
                <div class="filter-item">
                    <label for="tender_id">Tender ID</label>
                    <select name="tender_id[]" id="tender_id" class="form-control select2-multiple" multiple>
                        @foreach($tenderIds as $tenderId)
                            <option value="{{ $tenderId }}" {{ is_array(request('tender_id')) && in_array($tenderId, request('tender_id')) ? 'selected' : '' }}>{{ $tenderId }}</option>
                        @endforeach
                    </select>
                </div>
                
                <!-- New Contract Number Filter -->
                <div class="filter-item">
                    <label for="contract_number">Contract Number/Purchase Order</label>
                    <select name="contract_number[]" id="contract_number" class="form-control select2-multiple" multiple>
                        @foreach($contractNumbers as $contractNumber)
                            <option value="{{ $contractNumber }}" {{ is_array(request('contract_number')) && in_array($contractNumber, request('contract_number')) ? 'selected' : '' }}>{{ $contractNumber }}</option>
                        @endforeach
                    </select>
                </div>
