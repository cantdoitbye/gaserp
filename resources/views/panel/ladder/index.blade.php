...
<div class="main-container">
    ...
    <h1 class="page-title">Ladder Data Tracker</h1>

    <!-- Report Section -->
    <div class="report-section">
        <h2 class="report-title">Ladder Jobs Report</h2>
    ...
    <div class="content-card">
        <div class="action-buttons">
            <a href="{{ route('ladder.create') }}" class="btn btn-primary">
                <i class="fas fa-plus"></i> Add New Ladder Job
            </a>
            <a href="{{ route('ladder.import.form') }}" class="btn btn-success">
                <i class="fas fa-file-excel"></i> Import Excel
            </a>
            <a href="{{ route('ladder.export') }}" class="btn btn-secondary">
        ...
        <form id="search-form" action="{{ route('ladder.index') }}" method="GET" style="display: none;">
                ...
                <table class="data-table">
                    <thead>
                        <tr>
                            ...
                            <th class="basic-info-header">
                                <div class="header-search-container">
                                    <div class="header-title">
                                        <a href="{{ route('ladder.index', array_merge(request()->query(), ['sort' => 'agreement_date', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc'])) }}" class="sort-link">
                                            Agreement Date
                            ...
                            <th class="basic-info-header">
                                <div class="header-search-container">
                                    <div class="header-title">
                                        <a href="{{ route('ladder.index', array_merge(request()->query(), ['sort' => 'customer_name', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc'])) }}" class="sort-link">
                                            Name
                            ...
                            <th class="basic-info-header">
                                <div class="header-search-container">
                                    <div class="header-title">
                                        <a href="{{ route('ladder.index', array_merge(request()->query(), ['sort' => 'area', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc'])) }}" class="sort-link">
                                            Area
                                    ...
                                    <select name="area" class="header-select" onchange="updateFormAndSubmit(this)">
                                        ...
                                        @foreach(\App\Models\Hadder::getAreaOptions() as $key => $value)
                            ...
                            <th class="basic-info-header">
                                <div class="header-search-container">
                                    ...
                                    <select name="scheme" class="header-select" onchange="updateFormAndSubmit(this)">
                                        <option value="">-- All --</option>
                                        @foreach(\App\Models\Hadder::getSchemeOptions() as $key => $value)
                    ...
                    <tbody>
                        @forelse($ladders as $ladder)
                            <tr>
                                <td>{{ $ladder->agreement_date ? $ladder->agreement_date->format('d-m-Y') : 'N/A' }}</td>
                                <td>{{ $ladder->customer_no ?? 'N/A' }}</td>
                                <td>{{ $ladder->service_order_no ?? 'N/A' }}</td>
                                <td>{{ $ladder->application_no ?? 'N/A' }}</td>
                                <td><strong>{{ $ladder->customer_name }}</strong></td>
                                <td>{{ $ladder->contact_no ?? 'N/A' }}</td>
                                <td title="{{ $ladder->address }}">{{ Str::limit($ladder->address, 25) }}</td>
                                <td>
                                    @if($ladder->area)
                                        <span class="area-badge">{{ ucfirst(str_replace('_', ' ', $ladder->area)) }}</span>
                                    @else
                                        N/A
                                    @endif
                                </td>
                                <td>{{ $ladder->scheme ? ucfirst(str_replace('_', ' ', $ladder->scheme)) : 'N/A' }}</td>
                                <td>
                                    @if($ladder->SlaCalculatedDays !== null)
                                        <span class="{{ $ladder->SlaCalculatedDays > 30 ? 'text-danger' : ($ladder->SlaCalculatedDays > 15 ? 'text-warning' : 'text-success') }}">
                                            {{ $ladder->SlaCalculatedDays }}
                                        </span>
                                    @else
                                        N/A
                                    @endif
                                ...
                                <td>
                                    <div class="action-icons">
                                        <a href="{{ route('ladder.show', $ladder) }}" class="action-icon icon-info" title="View">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{ route('ladder.edit', $ladder) }}" class="action-icon icon-edit" title="Edit">
                                        ...
                                        <form action="{{ route('ladder.destroy', $ladder) }}" method="POST" class="d-inline">
                                            ...
                                            <button type="submit" class="action-icon icon-delete" title="Delete" 
                                                    onclick="return confirm('Are you sure you want to delete this Ladder job?')">
                        ...
                        @empty
                            <tr>
                                <td colspan="11" style="text-align: center; padding: 15px;">
                                    No Ladder jobs found. 
                                    <a href="{{ route('ladder.create') }}">Create the first Ladder job</a> or 
                                    <a href="{{ route('ladder.import.form') }}">import from Excel</a>.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                            ...
                            <tr>
                                <td>
                                    @if($ladder->connections_status)
                                        <span class="status-badge status-{{ strtolower(str_replace(['_', ' '], '-', $ladder->connections_status)) }}">
                                ...
                                <td>{{ $ladder->plb_name ?? 'N/A' }}</td>
                                <td>{{ $ladder->plb_date ? $ladder->plb_date->format('d-m-Y') : 'N/A' }}</td>
                                <td>{{ $ladder->pdt_date ? $ladder->pdt_date->format('d-m-Y') : 'N/A' }}</td>
                                <td>{{ $ladder->pdt_witness_by ?? 'N/A' }}</td>
                                <td>{{ $ladder->ground_connections_date ? $ladder->ground_connections_date->format('d-m-Y') : 'N/A' }}</td>
                                <td>{{ $ladder->ground_connections_witness_by ?? 'N/A' }}</td>
                                <td>{{ $ladder->isolation_name ?? 'N/A' }}</td>
                                <td>{{ $ladder->mmt_date ? $ladder->mmt_date->format('d-m-Y') : 'N/A' }}</td>
                                <td>{{ $ladder->mmt_witness_by ?? 'N/A' }}</td>
                                <td>{{ $ladder->conversion_technician_name ?? 'N/A' }}</td>
                                <td>{{ $ladder->conversion_date ? $ladder->conversion_date->format('d-m-Y') : 'N/A' }}</td>
                                <td>
                                    @if($ladder->conversion_status)
                                        <span class="status-badge status-{{ strtolower(str_replace(['_', ' '], '-', $ladder->conversion_status)) }}">
                                ...
                                <td>{{ $ladder->report_submission_date ? $ladder->report_submission_date->format('d-m-Y') : 'N/A' }}</td>
                                <td>{{ $ladder->meter_number ?? 'N/A' }}</td>
                                <td>{{ $ladder->ra_bill_no ?? 'N/A' }}</td>
                                <td title="{{ $ladder->remarks }}">{{ Str::limit($ladder->remarks, 30) }}</td>
                            </tr>
                        @empty
                            <tr>
        ...
        <div style="margin-top: 15px; display: flex; justify-content: center;">
            {{ $ladders->appends(request()->query())->links() }}
        </div>
    </div>
</div>
@endsection
