...
<style>
/* Enhanced styles for Ladder show view */
...
<div class="main-container">
   <div class="top-bar">
        <div class="search-box">
            ...
            {{-- <input type="text" placeholder="Search Ladder tracker records..."> --}}
        </div>
    ...
    <div class="job-details-container">
        ...
        <div class="job-header">
            <h1 class="job-title">Ladder Job Details - #{{ $ladder->id }}</h1>
            <div class="action-buttons">
                <a href="{{ route('ladder.index') }}" class="btn-custom btn-back">
                    <i class="fas fa-arrow-left"></i> Back to List
                </a>
                <a href="{{ route('ladder.edit', $ladder) }}" class="btn-custom btn-edit">
                    <i class="fas fa-edit"></i> Edit
                </a>
                <form action="{{ route('ladder.destroy', $ladder) }}" method="POST" class="d-inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn-custom btn-delete" onclick="return confirm('Are you sure you want to delete this Ladder job?')">
        ...
        <div class="details-grid">
            ...
            <div class="detail-section">
                ...
                <div class="info-row">
                    ...
                    <div class="info-value"><strong>{{ $ladder->service_order_no ?? 'N/A' }}</strong></div>
                </div>
                <div class="info-row">
                    <div class="info-label">Customer No:</div>
                    <div class="info-value">{{ $ladder->customer_no ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Application No:</div>
                    <div class="info-value">{{ $ladder->application_no ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Agreement Date:</div>
                    <div class="info-value">{{ $ladder->agreement_date ? $ladder->agreement_date->format('d-m-Y') : 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Area:</div>
                    <div class="info-value">{{ $ladder->area ? ucfirst($ladder->area) : 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Scheme:</div>
                    <div class="info-value">{{ $ladder->scheme ? ucfirst($ladder->scheme) : 'N/A' }}</div>
                </div>
            </div>
            ...
            <div class="detail-section">
                ...
                <div class="info-row">
                    ...
                    <div class="info-value"><strong>{{ $ladder->customer_name }}</strong></div>
                </div>
                <div class="info-row">
                    <div class="info-label">Contact Number:</div>
                    <div class="info-value">{{ $ladder->contact_no ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Address:</div>
                    <div class="info-value">
                        <div class="address-display">
                            {{ $ladder->address ?: 'No address provided' }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="detail-section">
                ...
                <div class="info-row">
                    ...
                    <div class="info-value">
                        @if($ladder->connections_status)
                            <span class="status-badge status-{{ strtolower(str_replace(' ', '-', $ladder->connections_status)) }}">
                                {{ ucfirst(str_replace('_', ' ', $ladder->connections_status)) }}
                            </span>
                        @else
                            N/A
                        @endif
                ...
                <div class="info-row">
                    ...
                    <div class="info-value">
                        @if($ladder->conversion_status)
                            <span class="status-badge status-{{ strtolower(str_replace(' ', '-', $ladder->conversion_status)) }}">
                                {{ ucfirst(str_replace('_', ' ', $ladder->conversion_status)) }}
                            </span>
                        @else
                            N/A
                        @endif
                ...
                <div class="info-row">
                    <div class="info-label">Ladder Type:</div>
                    <div class="info-value">{{ $ladder->ladder_type ? ucfirst($ladder->ladder_type) : 'N/A' }}</div>
                </div>
                @if($ladder->measurementType)
                    <div class="info-row">
                        <div class="info-label">Measurement Type:</div>
                        <div class="info-value">{{ $ladder->measurementType->name }}</div>
                    </div>
                @endif
            </div>
            ...
            <div class="detail-section">
                ...
                <div class="info-row">
                    ...
                    <div class="info-value">{{ $ladder->plb_name ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">PPT Witness By:</div>
                    <div class="info-value">{{ $ladder->pdt_witness_by ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Ground Connections Witness By:</div>
                    <div class="info-value">{{ $ladder->ground_connections_witness_by ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">MMT Witness By:</div>
                    <div class="info-value">{{ $ladder->mmt_witness_by ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Conversion Technician:</div>
                    <div class="info-value">{{ $ladder->conversion_technician_name ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Isolation Name:</div>
                    <div class="info-value">{{ $ladder->isolation_name ?? 'N/A' }}</div>
                </div>
            </div>
        </div>
            ...
            <div class="details-grid">
                <div>
                    <div class="info-row">
                        ...
                        <div class="info-value">{{ $ladder->plb_date ? $ladder->plb_date->format('d-m-Y') : 'N/A' }}</div>
                    </div>
                    <div class="info-row">
                        <div class="info-label">PPT Date:</div>
                        <div class="info-value">{{ $ladder->pdt_date ? $ladder->pdt_date->format('d-m-Y') : 'N/A' }}</div>
                    </div>
                    <div class="info-row">
                        <div class="info-label">Ground Connections Date:</div>
                        <div class="info-value">{{ $ladder->ground_connections_date ? $ladder->ground_connections_date->format('d-m-Y') : 'N/A' }}</div>
                    </div>
                </div>
                <div>
                    <div class="info-row">
                        <div class="info-label">MMT Date:</div>
                        <div class="info-value">{{ $ladder->mmt_date ? $ladder->mmt_date->format('d-m-Y') : 'N/A' }}</div>
                    </div>
                    <div class="info-row">
                        <div class="info-label">Conversion Date:</div>
                        <div class="info-value">{{ $ladder->conversion_date ? $ladder->conversion_date->format('d-m-Y') : 'N/A' }}</div>
                    </div>
                    <div class="info-row">
                        <div class="info-label">Report Submission Date:</div>
                        <div class="info-value">{{ $ladder->report_submission_date ? $ladder->report_submission_date->format('d-m-Y') : 'N/A' }}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="detail-section" style="margin-bottom: 25px;">
            ...
            <div class="info-row">
                ...
                <div class="info-value">{{ $ladder->meter_number ?? 'N/A' }}</div>
            </div>
            <div class="info-row">
                <div class="info-label">RA Bill No:</div>
                <div class="info-value">{{ $ladder->ra_bill_no ?? 'N/A' }}</div>
            </div>
            <div class="info-row">
                <div class="info-label">Remarks:</div>
                <div class="info-value">
                    <div style="background-color: white; padding: 12px; border-radius: 6px; border: 1px solid #e9ecef;">
                        {{ $ladder->remarks ?: 'No remarks available.' }}
                    </div>
                </div>
            </div>
        </div>
        ...
        @if($ladder->measurements_data && count($ladder->measurements_data) > 0)
            <div class="detail-section" style="margin-bottom: 25px;">
                <div class="section-title">
                ...
                @php
                    ...
                    if ($ladder->measurementType && $ladder->measurementType->measurement_fields) {
                        $measurementFields = $ladder->measurementType->measurement_fields;
        ...
        <div class="detail-section" style="margin-bottom: 25px;">
            ...
            @foreach($filesSections as $field => $config)
                @if($ladder->$field)
                    ...
                    <div class="file-item">
                        ...
                        <a href="{{ Storage::url($ladder->$field) }}" target="_blank" class="btn-custom btn-view">
            ...
            @foreach($multipleFileSections as $field => $config)
                @if($ladder->$field && count($ladder->$field) > 0)
                    @php $hasFiles = true; @endphp
                    @foreach($ladder->$field as $index => $file)
                        <div class="file-item">
                            <i class="fas fa-{{ $config['icon'] }} file-icon text-{{ $config['color'] }}"></i>
                            <span class="file-name">
                                {{ $config['label'] }} {{ count($ladder->$field) > 1 ? '(' . ($index + 1) . ')' : '' }}
                                @if(isset($file['name']))
                                    - {{ $file['name'] }}
                                @endif
                            </span>
        ...
        <div class="detail-section">
            ...
            <div class="info-row">
                ...
                <div class="info-value"><strong>#{{ $ladder->id }}</strong></div>
            </div>
            <div class="info-row">
                <div class="info-label">Created At:</div>
                <div class="info-value">{{ $ladder->created_at->format('d-m-Y H:i:s') }}</div>
            </div>
            <div class="info-row">
                <div class="info-label">Last Updated:</div>
                <div class="info-value">{{ $ladder->updated_at->format('d-m-Y H:i:s') }}</div>
            </div>
        </div>
    </div>
</div>
