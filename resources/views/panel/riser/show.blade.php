...
<style>
/* Enhanced styles for Riser show view */
...
<div class="main-container">
   <div class="top-bar">
        <div class="search-box">
            ...
            {{-- <input type="text" placeholder="Search Riser tracker records..."> --}}
        </div>
    ...
    <div class="job-details-container">
        ...
        <div class="job-header">
            <h1 class="job-title">Riser Job Details - #{{ $riser->id }}</h1>
            <div class="action-buttons">
                <a href="{{ route('riser.index') }}" class="btn-custom btn-back">
                    <i class="fas fa-arrow-left"></i> Back to List
                </a>
                <a href="{{ route('riser.edit', $riser) }}" class="btn-custom btn-edit">
                    <i class="fas fa-edit"></i> Edit
                </a>
                <form action="{{ route('riser.destroy', $riser) }}" method="POST" class="d-inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn-custom btn-delete" onclick="return confirm('Are you sure you want to delete this Riser job?')">
        ...
        <div class="details-grid">
            ...
            <div class="detail-section">
                ...
                <div class="info-row">
                    ...
                    <div class="info-value"><strong>{{ $riser->service_order_no ?? 'N/A' }}</strong></div>
                </div>
                <div class="info-row">
                    <div class="info-label">Customer No:</div>
                    <div class="info-value">{{ $riser->customer_no ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Application No:</div>
                    <div class="info-value">{{ $riser->application_no ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Agreement Date:</div>
                    <div class="info-value">{{ $riser->agreement_date ? $riser->agreement_date->format('d-m-Y') : 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Area:</div>
                    <div class="info-value">{{ $riser->area ? ucfirst($riser->area) : 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Scheme:</div>
                    <div class="info-value">{{ $riser->scheme ? ucfirst($riser->scheme) : 'N/A' }}</div>
                </div>
            </div>
            ...
            <div class="detail-section">
                ...
                <div class="info-row">
                    ...
                    <div class="info-value"><strong>{{ $riser->customer_name }}</strong></div>
                </div>
                <div class="info-row">
                    <div class="info-label">Contact Number:</div>
                    <div class="info-value">{{ $riser->contact_no ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Address:</div>
                    <div class="info-value">
                        <div class="address-display">
                            {{ $riser->address ?: 'No address provided' }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="detail-section">
                ...
                <div class="info-row">
                    ...
                    <div class="info-value">
                        @if($riser->connections_status)
                            <span class="status-badge status-{{ strtolower(str_replace(' ', '-', $riser->connections_status)) }}">
                ...
                <div class="info-row">
                    ...
                    <div class="info-value">
                        @if($riser->conversion_status)
                            <span class="status-badge status-{{ strtolower(str_replace(' ', '-', $riser->conversion_status)) }}">
                ...
                <div class="info-row">
                    <div class="info-label">Riser Type:</div>
                    <div class="info-value">{{ $riser->riser_type ? ucfirst($riser->riser_type) : 'N/A' }}</div>
                </div>
                @if($riser->measurementType)
                    <div class="info-row">
                        <div class="info-label">Measurement Type:</div>
                        <div class="info-value">{{ $riser->measurementType->name }}</div>
                    </div>
                @endif
            </div>
            ...
            <div class="detail-section">
                ...
                <div class="info-row">
                    ...
                    <div class="info-value">{{ $riser->plb_name ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">PPT Witness By:</div>
                    <div class="info-value">{{ $riser->pdt_witness_by ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Ground Connections Witness By:</div>
                    <div class="info-value">{{ $riser->ground_connections_witness_by ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">MMT Witness By:</div>
                    <div class="info-value">{{ $riser->mmt_witness_by ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Conversion Technician:</div>
                    <div class="info-value">{{ $riser->conversion_technician_name ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Isolation Name:</div>
                    <div class="info-value">{{ $riser->isolation_name ?? 'N/A' }}</div>
                </div>
            </div>
        </div>
            ...
            <div class="details-grid">
                <div>
                    <div class="info-row">
                        ...
                        <div class="info-value">{{ $riser->plb_date ? $riser->plb_date->format('d-m-Y') : 'N/A' }}</div>
                    </div>
                    <div class="info-row">
                        <div class="info-label">PPT Date:</div>
                        <div class="info-value">{{ $riser->pdt_date ? $riser->pdt_date->format('d-m-Y') : 'N/A' }}</div>
                    </div>
                    <div class="info-row">
                        <div class="info-label">Ground Connections Date:</div>
                        <div class="info-value">{{ $riser->ground_connections_date ? $riser->ground_connections_date->format('d-m-Y') : 'N/A' }}</div>
                    </div>
                </div>
                <div>
                    <div class="info-row">
                        <div class="info-label">MMT Date:</div>
                        <div class="info-value">{{ $riser->mmt_date ? $riser->mmt_date->format('d-m-Y') : 'N/A' }}</div>
                    </div>
                    <div class="info-row">
                        <div class="info-label">Conversion Date:</div>
                        <div class="info-value">{{ $riser->conversion_date ? $riser->conversion_date->format('d-m-Y') : 'N/A' }}</div>
                    </div>
                    <div class="info-row">
                        <div class="info-label">Report Submission Date:</div>
                        <div class="info-value">{{ $riser->report_submission_date ? $riser->report_submission_date->format('d-m-Y') : 'N/A' }}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        @if($riser->measurements_data && count($riser->measurements_data) > 0)
            <div class="detail-section" style="margin-bottom: 25px;">
                <div class="section-title">
                ...
                @php
                    ...
                    if ($riser->measurementType && $riser->measurementType->measurement_fields) {
                        $measurementFields = $riser->measurementType->measurement_fields;
        ...
        <div class="detail-section" style="margin-bottom: 25px;">
            ...
            @foreach($filesSections as $field => $config)
                @if($riser->$field)
                    ...
                    <div class="file-item">
                        ...
                        <a href="{{ Storage::url($riser->$field) }}" target="_blank" class="btn-custom btn-view">
            ...
            @foreach($multipleFileSections as $field => $config)
                @if($riser->$field && count($riser->$field) > 0)
                    @php $hasFiles = true; @endphp
                    @foreach($riser->$field as $index => $file)
                        <div class="file-item">
                            <i class="fas fa-{{ $config['icon'] }} file-icon text-{{ $config['color'] }}"></i>
                            <span class="file-name">
                                {{ $config['label'] }} {{ count($riser->$field) > 1 ? '(' . ($index + 1) . ')' : '' }}
                                @if(isset($file['name']))
                                    - {{ $file['name'] }}
                                @endif
                            </span>
        ...
        <div class="detail-section">
            ...
            <div class="info-row">
                ...
                <div class="info-value"><strong>#{{ $riser->id }}</strong></div>
            </div>
            <div class="info-row">
                <div class="info-label">Created At:</div>
                <div class="info-value">{{ $riser->created_at->format('d-m-Y H:i:s') }}</div>
            </div>
            <div class="info-row">
                <div class="info-label">Last Updated:</div>
                <div class="info-value">{{ $riser->updated_at->format('d-m-Y H:i:s') }}</div>
            </div>
        </div>
    </div>
</div>
