...
<style>
/* Enhanced styles for Commercial show view */
...
<div class="main-container">
   <div class="top-bar">
        <div class="search-box">
            ...
            {{-- <input type="text" placeholder="Search Commercial tracker records..."> --}}
        </div>
    ...
    <div class="job-details-container">
        ...
        <div class="job-header">
            <h1 class="job-title">Commercial Job Details - #{{ $commercial->id }}</h1>
            <div class="action-buttons">
                <a href="{{ route('commercial.index') }}" class="btn-custom btn-back">
                    <i class="fas fa-arrow-left"></i> Back to List
                </a>
                <a href="{{ route('commercial.edit', $commercial) }}" class="btn-custom btn-edit">
                    <i class="fas fa-edit"></i> Edit
                </a>
                <form action="{{ route('commercial.destroy', $commercial) }}" method="POST" class="d-inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn-custom btn-delete" onclick="return confirm('Are you sure you want to delete this Commercial job?')">
        ...
        <div class="details-grid">
            ...
            <div class="detail-section">
                ...
                <div class="info-row">
                    ...
                    <div class="info-value"><strong>{{ $commercial->service_order_no ?? 'N/A' }}</strong></div>
                </div>
                <div class="info-row">
                    <div class="info-label">Customer No:</div>
                    <div class="info-value">{{ $commercial->customer_no ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Application No:</div>
                    <div class="info-value">{{ $commercial->application_no ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Agreement Date:</div>
                    <div class="info-value">{{ $commercial->agreement_date ? $commercial->agreement_date->format('d-m-Y') : 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Area:</div>
                    <div class="info-value">{{ $commercial->area ? ucfirst($commercial->area) : 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Scheme:</div>
                    <div class="info-value">{{ $commercial->scheme ? ucfirst($commercial->scheme) : 'N/A' }}</div>
                </div>
            </div>
            ...
            <div class="detail-section">
                ...
                <div class="info-row">
                    ...
                    <div class="info-value"><strong>{{ $commercial->customer_name }}</strong></div>
                </div>
                <div class="info-row">
                    <div class="info-label">Contact Number:</div>
                    <div class="info-value">{{ $commercial->contact_no ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Address:</div>
                    <div class="info-value">
                        <div class="address-display">
                            {{ $commercial->address ?: 'No address provided' }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="detail-section">
                ...
                <div class="info-row">
                    ...
                    <div class="info-value">
                        @if($commercial->connections_status)
                            <span class="status-badge status-{{ strtolower(str_replace(' ', '-', $commercial->connections_status)) }}">
                                {{ ucfirst(str_replace('_', ' ', $commercial->connections_status)) }}
                            </span>
                        @else
                            N/A
                        @endif
                ...
                <div class="info-row">
                    ...
                    <div class="info-value">
                        @if($commercial->conversion_status)
                            <span class="status-badge status-{{ strtolower(str_replace(' ', '-', $commercial->conversion_status)) }}">
                                {{ ucfirst(str_replace('_', ' ', $commercial->conversion_status)) }}
                            </span>
                        @else
                            N/A
                        @endif
                ...
                <div class="info-row">
                    <div class="info-label">Commercial Type:</div>
                    <div class="info-value">{{ $commercial->commercial_type ? ucfirst($commercial->commercial_type) : 'N/A' }}</div>
                </div>
                @if($commercial->measurementType)
                    <div class="info-row">
                        <div class="info-label">Measurement Type:</div>
                        <div class="info-value">{{ $commercial->measurementType->name }}</div>
                    </div>
                @endif
            </div>
            ...
            <div class="detail-section">
                ...
                <div class="info-row">
                    ...
                    <div class="info-value">{{ $commercial->plb_name ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">PPT Witness By:</div>
                    <div class="info-value">{{ $commercial->pdt_witness_by ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Ground Connections Witness By:</div>
                    <div class="info-value">{{ $commercial->ground_connections_witness_by ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">MMT Witness By:</div>
                    <div class="info-value">{{ $commercial->mmt_witness_by ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Conversion Technician:</div>
                    <div class="info-value">{{ $commercial->conversion_technician_name ?? 'N/A' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label">Isolation Name:</div>
                    <div class="info-value">{{ $commercial->isolation_name ?? 'N/A' }}</div>
                </div>
            </div>
        </div>
            ...
            <div class="details-grid">
                <div>
                    <div class="info-row">
                        ...
                        <div class="info-value">{{ $commercial->plb_date ? $commercial->plb_date->format('d-m-Y') : 'N/A' }}</div>
                    </div>
                    <div class="info-row">
                        <div class="info-label">PPT Date:</div>
                        <div class="info-value">{{ $commercial->pdt_date ? $commercial->pdt_date->format('d-m-Y') : 'N/A' }}</div>
                    </div>
                    <div class="info-row">
                        <div class="info-label">Ground Connections Date:</div>
                        <div class="info-value">{{ $commercial->ground_connections_date ? $commercial->ground_connections_date->format('d-m-Y') : 'N/A' }}</div>
                    </div>
                </div>
                <div>
                    <div class="info-row">
                        <div class="info-label">MMT Date:</div>
                        <div class="info-value">{{ $commercial->mmt_date ? $commercial->mmt_date->format('d-m-Y') : 'N/A' }}</div>
                    </div>
                    <div class="info-row">
                        <div class="info-label">Conversion Date:</div>
                        <div class="info-value">{{ $commercial->conversion_date ? $commercial->conversion_date->format('d-m-Y') : 'N/A' }}</div>
                    </div>
                    <div class="info-row">
                        <div class="info-label">Report Submission Date:</div>
                        <div class="info-value">{{ $commercial->report_submission_date ? $commercial->report_submission_date->format('d-m-Y') : 'N/A' }}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="detail-section" style="margin-bottom: 25px;">
            ...
            <div class="info-row">
                ...
                <div class="info-value">{{ $commercial->meter_number ?? 'N/A' }}</div>
            </div>
            <div class="info-row">
                <div class="info-label">RA Bill No:</div>
                <div class="info-value">{{ $commercial->ra_bill_no ?? 'N/A' }}</div>
            </div>
            <div class="info-row">
                <div class="info-label">Remarks:</div>
                <div class="info-value">
                    <div style="background-color: white; padding: 12px; border-radius: 6px; border: 1px solid #e9ecef;">
                        {{ $commercial->remarks ?: 'No remarks available.' }}
                    </div>
                </div>
            </div>
        </div>
        ...
        @if($commercial->measurements_data && count($commercial->measurements_data) > 0)
            <div class="detail-section" style="margin-bottom: 25px;">
                <div class="section-title">
                ...
                @php
                    ...
                    if ($commercial->measurementType && $commercial->measurementType->measurement_fields) {
                        $measurementFields = $commercial->measurementType->measurement_fields;
        ...
        <div class="detail-section" style="margin-bottom: 25px;">
            ...
            @foreach($filesSections as $field => $config)
                @if($commercial->$field)
                    ...
                    <div class="file-item">
                        ...
                        <a href="{{ Storage::url($commercial->$field) }}" target="_blank" class="btn-custom btn-view">
            ...
            @foreach($multipleFileSections as $field => $config)
                @if($commercial->$field && count($commercial->$field) > 0)
                    @php $hasFiles = true; @endphp
                    @foreach($commercial->$field as $index => $file)
                        <div class="file-item">
                            <i class="fas fa-{{ $config['icon'] }} file-icon text-{{ $config['color'] }}"></i>
                            <span class="file-name">
                                {{ $config['label'] }} {{ count($commercial->$field) > 1 ? '(' . ($index + 1) . ')' : '' }}
                                @if(isset($file['name']))
                                    - {{ $file['name'] }}
                                @endif
                            </span>
        ...
        <div class="detail-section">
            ...
            <div class="info-row">
                ...
                <div class="info-value"><strong>#{{ $commercial->id }}</strong></div>
            </div>
            <div class="info-row">
                <div class="info-label">Created At:</div>
                <div class="info-value">{{ $commercial->created_at->format('d-m-Y H:i:s') }}</div>
            </div>
            <div class="info-row">
                <div class="info-label">Last Updated:</div>
                <div class="info-value">{{ $commercial->updated_at->format('d-m-Y H:i:s') }}</div>
            </div>
        </div>
    </div>
</div>
