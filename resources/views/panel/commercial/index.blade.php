{{-- panel/commercial/index.blade.php --}}
...
<div class="main-container">
    ...
    <h1 class="page-title">Commercial Data Tracker</h1>

    <!-- Report Section -->
    <div class="report-section">
        <h2 class="report-title">Commercial Jobs Report</h2>
    ...
    <div class="content-card">
        <div class="action-buttons">
            <a href="{{ route('commercial.create') }}" class="btn btn-primary">
                <i class="fas fa-plus"></i> Add New Commercial Job
            </a>
            <a href="{{ route('commercial.import.form') }}" class="btn btn-success">
                <i class="fas fa-file-excel"></i> Import Excel
            </a>
            <a href="{{ route('commercial.export') }}" class="btn btn-secondary">
        ...
        <form id="search-form" action="{{ route('commercial.index') }}" method="GET" style="display: none;">
                ...
                <table class="data-table">
                    <thead>
                        <tr>
                            ...
                            <th class="basic-info-header">
                                <div class="header-search-container">
                                    <div class="header-title">
                                        <a href="{{ route('commercial.index', array_merge(request()->query(), ['sort' => 'agreement_date', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc'])) }}" class="sort-link">
                                            Agreement Date
                            ...
                            <th class="basic-info-header">
                                <div class="header-search-container">
                                    <div class="header-title">
                                        <a href="{{ route('commercial.index', array_merge(request()->query(), ['sort' => 'customer_name', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc'])) }}" class="sort-link">
                                            Name
                            ...
                            <th class="basic-info-header">
                                <div class="header-search-container">
                                    <div class="header-title">
                                        <a href="{{ route('commercial.index', array_merge(request()->query(), ['sort' => 'area', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc'])) }}" class="sort-link">
                                            Area
                    ...
                    <tbody>
                        @forelse($commercials as $commercial)
                            <tr>
                                <td>{{ $commercial->agreement_date ? $commercial->agreement_date->format('d-m-Y') : 'N/A' }}</td>
                                <td>{{ $commercial->customer_no ?? 'N/A' }}</td>
                                <td>{{ $commercial->service_order_no ?? 'N/A' }}</td>
                                <td>{{ $commercial->application_no ?? 'N/A' }}</td>
                                <td><strong>{{ $commercial->customer_name }}</strong></td>
                                <td>{{ $commercial->contact_no ?? 'N/A' }}</td>
                                <td title="{{ $commercial->address }}">{{ Str::limit($commercial->address, 25) }}</td>
                                <td>
                                    @if($commercial->area)
                                        <span class="area-badge">{{ ucfirst(str_replace('_', ' ', $commercial->area)) }}</span>
                                    @else
                                        N/A
                                    @endif
                                </td>
                                <td>{{ $commercial->scheme ? ucfirst(str_replace('_', ' ', $commercial->scheme)) : 'N/A' }}</td>
                                <td>
                                    @if($commercial->SlaCalculatedDays !== null)
                                        <span class="{{ $commercial->SlaCalculatedDays > 30 ? 'text-danger' : ($commercial->SlaCalculatedDays > 15 ? 'text-warning' : 'text-success') }}">
                                            {{ $commercial->SlaCalculatedDays }}
                                        </span>
                                    @else
                                        N/A
                                    @endif
                                ...
                                <td>
                                    <div class="action-icons">
                                        <a href="{{ route('commercial.show', $commercial) }}" class="action-icon icon-info" title="View">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{ route('commercial.edit', $commercial) }}" class="action-icon icon-edit" title="Edit">
                                        ...
                                        <form action="{{ route('commercial.destroy', $commercial) }}" method="POST" class="d-inline">
                                            ...
                                            <button type="submit" class="action-icon icon-delete" title="Delete" 
                                                    onclick="return confirm('Are you sure you want to delete this Commercial job?')">
                        ...
                        @empty
                            <tr>
                                <td colspan="11" style="text-align: center; padding: 15px;">
                                    No Commercial jobs found. 
                                    <a href="{{ route('commercial.create') }}">Create the first Commercial job</a> or 
                                    <a href="{{ route('commercial.import.form') }}">import from Excel</a>.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                            ...
                            <tr>
                                <td>
                                    @if($commercial->connections_status)
                                        <span class="status-badge status-{{ strtolower(str_replace(['_', ' '], '-', $commercial->connections_status)) }}">
                                ...
                                <td>{{ $commercial->plb_name ?? 'N/A' }}</td>
                                <td>{{ $commercial->plb_date ? $commercial->plb_date->format('d-m-Y') : 'N/A' }}</td>
                                <td>{{ $commercial->pdt_date ? $commercial->pdt_date->format('d-m-Y') : 'N/A' }}</td>
                                <td>{{ $commercial->pdt_witness_by ?? 'N/A' }}</td>
                                <td>{{ $commercial->ground_connections_date ? $commercial->ground_connections_date->format('d-m-Y') : 'N/A' }}</td>
                                <td>{{ $commercial->ground_connections_witness_by ?? 'N/A' }}</td>
                                <td>{{ $commercial->isolation_name ?? 'N/A' }}</td>
                                <td>{{ $commercial->mmt_date ? $commercial->mmt_date->format('d-m-Y') : 'N/A' }}</td>
                                <td>{{ $commercial->mmt_witness_by ?? 'N/A' }}</td>
                                <td>{{ $commercial->conversion_technician_name ?? 'N/A' }}</td>
                                <td>{{ $commercial->conversion_date ? $commercial->conversion_date->format('d-m-Y') : 'N/A' }}</td>
                                <td>
                                    @if($commercial->conversion_status)
                                        <span class="status-badge status-{{ strtolower(str_replace(['_', ' '], '-', $commercial->conversion_status)) }}">
                                ...
                                <td>{{ $commercial->report_submission_date ? $commercial->report_submission_date->format('d-m-Y') : 'N/A' }}</td>
                                <td>{{ $commercial->meter_number ?? 'N/A' }}</td>
                                <td>{{ $commercial->ra_bill_no ?? 'N/A' }}</td>
                                <td title="{{ $commercial->remarks }}">{{ Str::limit($commercial->remarks, 30) }}</td>
                            </tr>
                        @empty
                            <tr>
        ...
        <div style="margin-top: 15px; display: flex; justify-content: center;">
            {{ $commercials->appends(request()->query())->links() }}
        </div>
    </div>
</div>
@endsection
