    ...
    <div class="container-fluid">
        <div class="py-4">
            <div class="d-sm-flex justify-content-between mb-3 mc-flex">
                <h2 class="page-title">{{ isDeleted() }}  Tour types (<span id="total_record"></span>)</h2>
                @if (isset($permissions['tour_type']) && in_array('add', $permissions['tour_type']))
                <button class="btn btn-primary mw-100" id="create_new_tour_type_offer">Add
                </button>
                @endif
            </div>
            ...
            <?php $deleted_text = "View deleted tour types"; ?>
            @include('admin.datatable.restore_button')
        </div>
    </div>
    @include('admin.master-data.tourType.add')
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            table.DataTable({
                ...
                ajax: {
                    url: globalSiteUrl + "/admin/master/tour-type",
                    data: function (data) {
        ...
        //for Add Model
        $(document).on('click', '#create_new_tour_type_offer', function () {
            $('label.error').hide();
            $('#tour_type_id').val('');
            $('#tour_type_from').trigger("reset");
            $('#modelHeaderTitle').html("Add tour type");
        ...
        $(document).on('click', '.editRow', function () {
            let id = $(this).data('id');
            $.get(globalSiteUrl + "/admin/master/tour-type/edit/" + id, function (data) {
                $('label.error').hide();
                $('*').removeClass('error');
        ...
        $(document).on('submit', '#tour_type_from', function (e) {
            e.preventDefault();
            let form = $(this);
            if (form.valid()) {
                ...
                $.ajax({
                    data: form.serialize(),
                    url: globalSiteUrl + "/admin/master/tour-type/add",
                    type: "POST",
                    dataType: 'json',
        ...
        $(document).on('click', '.changeStatus', function () {
            ...
            $('#activeModalHeading').html('Active tour type');
            ...
            if (status == 0) {
                $('#activeModalHeading').html('Inactive tour type');
        ...
        $(document).on('click', '#activeBtn', function () {
            ...
            $.ajax({
                method: 'POST',
                url: globalSiteUrl + '/admin/master/tour-type/status',
        ...
        $(document).on('click', '.deleteRow', function () {
            ...
            $('#deleteContent').html('Are you sure you want to delete <span id="deleteNameLabel" class="text-primary">' + name + '</span> tour type?')
            if (type == 2) {

                $('#deleteModalHeading').html('Restore tour type');
                $('#deleteBtn').html('Restore')
                $('#deleteContent').html('Are you sure you want to restore <span id="deleteNameLabel" class="text-primary">' + name + '</span> tour type?')
            }
            if (type == 3) {
                $('#deleteModalHeading').html('Permanent Delete Machine');
                $('#deleteContent').html('Are you sure you want to permanent delete <span id="deleteNameLabel" class="text-primary">' + name + '</span> tour type?')
            }
